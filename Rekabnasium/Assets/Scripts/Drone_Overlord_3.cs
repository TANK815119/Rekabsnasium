...
public class Drone_Overlord_3 : Agent
{
    ...
    [SerializeField] private float decisionTImeMax = 1f; //how much time till another decisiona made
    ...
    public override void OnEpisodeBegin()
    {
        ...
        //make new POI
        POI.localPosition = new Vector3(Random.Range(-randRange, randRange), Random.Range(-randRange, 0f), Random.Range(-randRange, randRange));
    }
    ...
    public override void CollectObservations(VectorSensor sensor) //TOTAL: 12
    {
        ...
        //accuracy is scaled to how well the POI is seen
        Vector3 relativePosition = POI.position - drone.position;
        Vector3 targetDir = (POI.position - target.position).normalized;
        //float yawError = Mathf.Abs(Vector3.Angle(drone.forward, targetDir));
        float yawError = Mathf.Abs(Vector3.SignedAngle(target.forward, targetDir, target.up));
        float yawMult = Mathf.Pow(1f - (yawError / 180f), 2f); //Values:0-1
        sensor.AddObservation(relativePosition * yawMult); // 3 values
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        Vector3 targetDir = (POI.position - target.position).normalized;
        float yawError = Vector3.SignedAngle(target.forward, targetDir, target.up);
        float currentYaw = target.eulerAngles.y; // Get the current yaw in world space
        float yaw = currentYaw + yawError; // Add the yaw offset
    ...
    private void CalculateReward()
    {
        ...
        //calculate reward from looking at POI
        //float yawError = Mathf.Abs(Mathf.DeltaAngle(drone.eulerAngles.y, target.eulerAngles.y)); //value: 0-180
        Vector3 targetDir = (POI.position - target.position).normalized;
        float yawError = Mathf.Abs(Vector3.SignedAngle(target.forward, targetDir, target.up));
        //float yawError = Mathf.Abs(Vector3.Angle(drone.forward, targetDir)); //theres actually no yaw involved here, whcih may fuck it up
        float yawReward = (2f * Mathf.Pow(1f - (yawError / 180f), 2f)) - 1f; //Values:-1-1
        //Debug.Log(yawReward);

        //caulculate reward from POI being on screen
        ...
        //}
        //I've decided not to do this because its hard and not very useful

        //weight rewards properly
        float combineRewards = distanceReward * 0.25f + yawReward * 0.75f;
